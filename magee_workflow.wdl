...
task run_null_model {
	...
	runtime {
		docker: "quay.io/large-scale-gxe-methods/magee-workflow@sha256:39006ef3834feebad4d62af2fe04e2a4807bafde3394b74ce45c093ebdb42436"
		memory: "${memory} GB"
...
task run_gwis_agg {
  ...
  String AF_strata_range
	...
	command <<<
		...
		Rscript /MAGEE_prep.R ${null_modelfile} "${exposure_names}" "${int_covar_names}" ${gdsfile} ${groupfile} "${gds_filter}" "${meta_file_prefix}" "${use_minor_allele}" "${AF_strata_range}" ${threads}
