workflow run_MAGEE_meta {
  ...
  Array[String] meta_file_prefix = ["NULL"]
  File? group_file
  ...
  String? out_prefix = "meta_output.txt"
  String? interaction
  Int? threads = 1
  Int? memory = 10
  Int? disk = 50
  ...
  parameter_meta {
    metafiles: "Either a list of files (for single-variant) or a list of tarballs (rare variant) per cohort of meta-analysis output files from the run_MAGEE workflow containing summary statistics and covariance matrix for meta-analysis. The names of the files are set by the 'meta_file_prefix' parameter."
    meta_file_prefix: "List defining the prefix of metafiles for rare variant test."
...
task run_meta {
  ...
  command <<<
    ...
    Rscript /MAGEE_META.R "${sep=' ' metafiles}" "${interaction}" "${out_prefix}" "${threads}" "${group_file}" "${group_file_sep}" "${E_match}" "${MAF_range}" "${MAF_weights_beta}" "${miss_cutoff}" "${method}" "${tests}" "${use_minor_allele}" "${cohort_group_idx}" "${sep= ' 'meta_file_prefix}"
  >>>
